...
Run API v1 coverage script (requires app running on `SOVEREIGN_E2E_BASE_URL` or **`http://localhost:3007`** — same host as `NEXTAUTH_URL`):

```bash
cd "$(git rev-parse --show-toplevel)/apps/sovereign"
