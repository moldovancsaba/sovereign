# {sentinelsquad} Delivery Roadmap
...
`{sentinelsquad}` is a desktop-first AI agentic IDE and unified chat product.
...
- `{sentinelsquad}` as the source of truth for agents, threads, tasks, approvals, and memory
...
1. `{sentinelsquad}` owns orchestration.
...
- `{sentinelsquad}` can create/open a local workspace session
...
Goal:
- achieve a working local `{sentinelsquad}` that can run a controlled multi-agent workflow inside one command center
...
Goal:
- make `{sentinelsquad}` capable of discovering and evaluating new tools/models without losing governance
...
Every `{sentinelsquad}` delivery issue must include:
