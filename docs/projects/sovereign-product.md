...
`{sovereign}` is the local-first desktop product for multi-agent software delivery and long-term project memory.
...
- MLX
- launchd
- custom `{sovereign}` orchestration, memory, and tool-execution layers
...
- repo root (e.g. `.../sovereign` or `.../sentinelsquad` before full rename)
