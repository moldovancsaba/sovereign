...
export async function syncNexusSeminarToSentinelSquadAction() {
  ...
  const summary = [
    `{sovereign} orchestration sync (${last.ok ? "PASS" : "FAIL"})`,
  ...
  const task = await enqueueTask({
    ...
    title: "Sovereign orchestration follow-up: validate and report",
    threadId: thread.id,
    createdById: userId,
  ...
  await createMessage({
    threadId: thread.id,
    authorType: "SYSTEM",
    content: `Sovereign orchestration follow-up queued for @${controller.key} (task ${task.id}).`,
    meta: {
