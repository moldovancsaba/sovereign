...
import { retrieveProjectMemory } from "@/lib/memory";
...
export async function ideProjectMemoryAction(formData: FormData) {
  const session = await requireSession();
  if (!session) throw new Error("Unauthorized");
  const projectSessionId = String(formData.get("projectSessionId") || "").trim();
  const query = String(formData.get("query") || "").trim();
  if (!projectSessionId) throw new Error("projectSessionId is required.");
