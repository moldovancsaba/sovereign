...
export function IdeClient(props: {
  ...
  initialProjectMemory: ProjectMemoryEntry[];
  agents: RuntimeAgent[];
}) {
  ...
  const [memoryEntries, setMemoryEntries] = useState<ProjectMemoryEntry[]>(props.initialProjectMemory);
  ...
  function openProjectSession(nextRelPath: string, displayName?: string) {
    startTransition(async () => {
      try {
        ...
        const memoryFd = new FormData();
        memoryFd.set("projectSessionId", result.id);
        const nextMemory = await ideProjectMemoryAction(memoryFd);
        setMemoryEntries(
          nextMemory.map((entry) => ({
            ...entry,
    ...
    startTransition(async () => {
      try {
        ...
        if (activeProjectSessionId === sessionId) {
          ...
          const memoryFd = new FormData();
          memoryFd.set("projectSessionId", props.rootProjectSession.id);
          const nextMemory = await ideProjectMemoryAction(memoryFd);
          setMemoryEntries(
            nextMemory.map((entry) => ({
              ...entry,
  ...
  function loadProjectMemory(query = memoryQuery) {
    startTransition(async () => {
      try {
        setStatus("Loading project memory...");
        const fd = new FormData();
        fd.set("projectSessionId", activeProjectSessionId);
        if (query.trim()) fd.set("query", query.trim());
        const result = await ideProjectMemoryAction(fd);
        setMemoryEntries(
          result.map((entry) => ({
            ...entry,
            createdAt: new Date(entry.createdAt).toISOString(),
        ...
        setStatus("Project memory loaded.");
      } catch (err) {
        setStatus(err instanceof Error ? err.message : String(err));
      }
    });
        ...
        <div className="ds-panel-deep p-3">
          <div className="mb-2 flex items-center justify-between">
            ...
            <button
              ...
              disabled={isPending}
              className="rounded-md border border-cyan-300/30 bg-cyan-300/10 px-3 py-1 text-xs disabled:opacity-50"
            >
              Refresh
            </button>
          </div>
          <div className="mb-2 flex gap-2">
            <input
              ...
              placeholder="Search memory for this project session"
              className="w-full rounded-md border border-white/20 bg-black/40 px-3 py-2 text-xs"
            />
            <button
              type="button"
              onClick={() => loadProjectMemory(memoryQuery)}
              disabled={isPending}
              className="rounded-md border border-white/20 bg-black/30 px-3 py-2 text-xs disabled:opacity-50"
            >
              Search
            </button>
          </div>
          <div className="mb-2 text-xs text-white/60">
            ...
            {projectSessions.find((session) => session.id === activeProjectSessionId)?.displayName || "workspace"}
          </div>
          <div className="space-y-2">
            {memoryEntries.length ? (
              memoryEntries.map((entry) => (
                <div
                  key={entry.id}
                  className="rounded-lg border border-white/10 bg-black/30 p-3 text-xs text-white/80"
                >
                  ...
                  <div className="mt-2 text-white/70">{entry.summary}</div>
                  <div className="mt-2 text-[11px] text-white/50">
                    {new Date(entry.updatedAt).toLocaleString()}
                    {entry.taskId ? ` · task=${entry.taskId.slice(0, 8)}` : ""}
                    {entry.sourceMessageId ? ` · msg=${entry.sourceMessageId.slice(0, 8)}` : ""}
                  </div>
                </div>
              ))
            ) : (
              <div className="rounded-lg border border-white/10 bg-black/30 p-3 text-xs text-white/60">
                No captured memory for this project session yet.
              </div>
            )}
          </div>
        </div>
