...
export default async function DashboardPage() {
  ...
  return (
    <Shell
      ...
      subtitle={
        meta
          ? `Local runtime with optional GitHub planning sync (${meta.owner}/projects/${meta.number} · product=${dashboardProduct})`
          : githubBoardEnabled
          ? "Local runtime with optional GitHub planning sync enabled."
          : "Runtime health, orchestrator lease, and operator telemetry — GitHub optional."
      }
    >
      <>
        {boardError ? (
          <div className="mb-4 rounded-2xl border border-white/12 bg-white/5 p-5 text-sm text-white/80">
            ...
            <div className="mt-4 opacity-85">
              GitHub is optional here. The local {"{sovereign}"} runtime is still available.
            </div>
          </div>
        ) : null}
        {!githubBoardEnabled ? (
          ...
          {localError ? (
            <div className="mb-4 rounded-2xl border border-amber-300/25 bg-amber-200/10 p-5 text-sm text-amber-50">
              ...
              <div className="mt-3 text-amber-100/80">
                Remediation: verify local DB migrations (`cd apps/sovereign && npx prisma migrate deploy`),
