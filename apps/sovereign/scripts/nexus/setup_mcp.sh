...
SENTINELSQUAD_PATH="${SENTINELSQUAD_PATH:-${_DEFAULT_APP_ROOT}}"
...
MCP_JSON=$(cat <<JSON
{
  "mcpServers": {
    "nexus-orchestrator": {
      "command": "python3",
      "args": ["$SENTINELSQUAD_PATH/scripts/nexus/mcp_server_py.py"],
      "env": {
        "CHATDEV_PATH": "$CHATDEV_PATH",
        "OLLAMA_HOST": "http://127.0.0.1:11434",
        "PYTHONPATH": "$SENTINELSQUAD_PATH"
      },
      "disabled": false
    },
    "local-terminal": {
      "command": "bash",
      "args": ["-c", "cd $SENTINELSQUAD_PATH && exec $SHELL"],
      "disabled": false
    }
  }
}
