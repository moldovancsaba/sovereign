...
const RAW_AGENT_KEY =
  argValue("--agent") || process.env.SENTINELSQUAD_WORKER_AGENT_KEY || null;
const POLL_MS = Number(process.env.SENTINELSQUAD_WORKER_POLL_MS || "1200");
const WORKER_MODEL = process.env.SENTINELSQUAD_WORKER_MODEL || null;
const WORKER_HOST = process.env.SENTINELSQUAD_WORKER_HOST || os.hostname();
...
const GITHUB_REPO_OWNER =
  process.env.SENTINELSQUAD_GITHUB_REPO_OWNER || "moldovancsaba";
const GITHUB_REPO_NAME =
  process.env.SENTINELSQUAD_GITHUB_REPO_NAME || "sentinelsquad";
...
const DEFAULT_MAX_ATTEMPTS = Number(process.env.SENTINELSQUAD_TASK_MAX_ATTEMPTS || "3");
const RETRY_BASE_MS = Number(process.env.SENTINELSQUAD_TASK_RETRY_BASE_MS || "5000");
const RETRY_MAX_MS = Number(process.env.SENTINELSQUAD_TASK_RETRY_MAX_MS || "300000");
const RETRY_JITTER_MS = Number(process.env.SENTINELSQUAD_TASK_RETRY_JITTER_MS || "750");
const REQUEST_TIMEOUT_MS = Number(
  process.env.SENTINELSQUAD_WORKER_REQUEST_TIMEOUT_MS || "60000"
);
const SHELL_STREAM_FLUSH_CHARS = clampInt(
  process.env.SENTINELSQUAD_SHELL_STREAM_FLUSH_CHARS || "1200",
  1200,
  200,
  4000
);
const SHELL_STREAM_MESSAGE_MAX_CHARS = clampInt(
  process.env.SENTINELSQUAD_SHELL_STREAM_MESSAGE_MAX_CHARS || "1600",
  1600,
  200,
  6000
);
const SHELL_ARTIFACT_SNIPPET_MAX_CHARS = clampInt(
  process.env.SENTINELSQUAD_SHELL_ARTIFACT_SNIPPET_MAX_CHARS || "4000",
  4000,
  500,
  24000
);
const ISSUE_EVIDENCE_MAX_ATTEMPTS = clampInt(
  process.env.SENTINELSQUAD_ISSUE_EVIDENCE_MAX_ATTEMPTS || "3",
  3,
  1,
  6
);
const ISSUE_EVIDENCE_RETRY_BASE_MS = clampInt(
  process.env.SENTINELSQUAD_ISSUE_EVIDENCE_RETRY_BASE_MS || "1000",
  1000,
  250,
  60_000
);
const ISSUE_EVIDENCE_RETRY_MAX_MS = clampInt(
  process.env.SENTINELSQUAD_ISSUE_EVIDENCE_RETRY_MAX_MS || "15000",
  15_000,
  ISSUE_EVIDENCE_RETRY_BASE_MS,
  300_000
);
const OUTPUT_DLP_MODE = resolveDlpMode(process.env.SENTINELSQUAD_DLP_MODE);
const ORCHESTRATOR_LEASE_ID =
  process.env.SENTINELSQUAD_ORCHESTRATOR_LEASE_ID || "sentinelsquad-primary-orchestrator";
const ORCHESTRATOR_LEASE_TTL_MS = clampInt(
  process.env.SENTINELSQUAD_ORCHESTRATOR_LEASE_TTL_MS || "20000",
  20_000,
  5_000,
  300_000
);
const ORCHESTRATOR_STALE_RUNNING_MS = clampInt(
  process.env.SENTINELSQUAD_ORCHESTRATOR_STALE_RUNNING_MS || String(Math.max(ORCHESTRATOR_LEASE_TTL_MS * 2, 30_000)),
  Math.max(ORCHESTRATOR_LEASE_TTL_MS * 2, 30_000),
  ORCHESTRATOR_LEASE_TTL_MS,
  3_600_000
);
const HANDOFF_INFERRED_MAX = clampInt(
  process.env.SENTINELSQUAD_HANDOFF_INFERRED_MAX || "3",
  3,
  1,
  10
);
...
const DRIFT_STATUS_CACHE_TTL_MS = clampInt(
  process.env.SENTINELSQUAD_DRIFT_STATUS_CACHE_TTL_MS || "15000",
  15_000,
  1_000,
  300_000
);
...
function readSentinelSquadSettingsFile() {
  try {
    const raw = fs.readFileSync(SETTINGS_FILE, "utf8");
...
function readCommandAccessEntries() {
  const parsed = readSentinelSquadSettingsFile();
...
async function maintainOrchestratorLease(reason) {
  ...
  if (outcome.held) {
    ...
    if (becameHolder || outcome.code !== "LEASE_RENEWED") {
      console.log(
        `[sentinelsquad-worker] lease ${outcome.code.toLowerCase()} owner=${ORCHESTRATOR_OWNER_ID} ttlMs=${ORCHESTRATOR_LEASE_TTL_MS}`
      );
    }
    return true;
  }
  ...
  if (
    ...
    console.log(`[sentinelsquad-worker] ${outcome.message}`);
  }
  return false;
}
...
async function taskIntakeDecision(agentKey, db = prisma) {
  ...
  if (!agent) {
    return {
      ...
      error: `Agent @${agentKey} is not registered in SentinelSquad.`
    };
  }
  if (!agent.enabled) {
    return {
...
async function ghGraphQL(query, variables) {
  if (!GITHUB_TOKEN) {
    throw new Error("Missing SENTINELSQUAD_GITHUB_TOKEN for board grounding.");
  }
  ...
  try {
    ...
    if (!currentProjectItem) {
      const value = {
        ...
        statusName: null,
        reason: `Issue #${issueNum} is not linked to SentinelSquad project ${meta?.title || "(unknown project)"}`
      };
...
function buildIssueEvidenceCommentBody(params) {
  ...
  return [
    `<!-- sentinelsquad-evidence:${evidenceKey} -->`,
    "SentinelSquad runtime evidence",
    `- task: \`${task.id}\``,
    `- outcome: \`${outcome}\``,
    `- agent: \`@${task.agentKey}\``,
        ...
        messages: [
          {
            role: "system",
            content:
              `You are ${config.displayName}, a pragmatic coding agent in SentinelSquad. Reply with concise, actionable output. If grounding data is provided, use only that data for status claims and explicitly avoid guessing.`
          },
        ...
        messages: [
          {
            role: "system",
            content:
              `You are ${config.displayName}, a pragmatic operations agent in SentinelSquad. Be concise and actionable. If grounding data is provided, use only that data for status claims and do not guess.`
          },
          { role: "user", content: `${userPrompt}${groundingBlock}` }
        ]
      })
    ...
    if (call.tool === "shell.exec") {
      ...
      const enqueueStreamOp = (op) => {
        streamState.queue = streamState.queue
          ...
          .catch((err) => {
            console.warn(
              `[sentinelsquad-worker] shell stream publish failed task=${task.id} call=${call.id}`,
              shortError(err)
            );
          });
  ...
  const resolveMemoryPromptForQuery = async (queryOverride = null) => {
    ...
    const pending = (async () => {
      ...
      try {
        ...
        const indexedRows = buildTaskMemoryIndexRows(messages, {
        ...
        const promptBlock = buildTaskMemoryPromptBlock(retrieval.snippets);
...
async function processTask(task) {
  ...
  try {
    ...
    if (failure.code === "LEASE_NOT_HELD" || failure.code === "TRANSITION_DENIED") {
      console.warn(
        `[sentinelsquad-worker] skipped task completion mutation for ${task.id}: ${failure.message}`
      );
      return;
    }
...
function requestShutdown(signal) {
  ...
  isShuttingDown = true;
  console.log(`[sentinelsquad-worker] received ${signal}; draining loop and releasing lease.`);
}
...
async function loop() {
  if (!RAW_AGENT_KEY) {
    throw new Error(
      "SENTINELSQUAD_WORKER_AGENT_KEY is required for control-plane worker startup and must reference an ALPHA agent."
    );
  }
  ...
  console.log(
    `[sentinelsquad-worker] started. agent=${WORKER_AGENT_KEY || "ANY"} role=${WORKER_CONTROL_ROLE || "UNSCOPED"} claimScope=${CLAIM_ALL_TASKS ? "ALL" : "FILTERED"} poll=${POLL_MS}ms owner=${ORCHESTRATOR_OWNER_ID} lease=${ORCHESTRATOR_LEASE_ID}`
  );
  ...
  for (; !isShuttingDown;) {
    try {
      ...
      if (nowMs - lastRecoveryMs >= Math.max(ORCHESTRATOR_STALE_RUNNING_MS, 5000)) {
        ...
        if (recovered > 0) {
          console.log(`[sentinelsquad-worker] recovered ${recovered} stale running task(s).`);
        }
      ...
      console.log(`[sentinelsquad-worker] claimed ${task.id} agent=${task.agentKey}`);
      ...
      const renewTimer = setInterval(() => {
        ...
        void maintainOrchestratorLease("task-heartbeat").catch((err) => {
          console.error("[sentinelsquad-worker] lease heartbeat failed", err);
        });
      }, renewIntervalMs);
      ...
      console.error("[sentinelsquad-worker] error", e);
      await sleep(POLL_MS);
    }
  }
}
...
loop()
  .catch((e) => {
    console.error("[sentinelsquad-worker] fatal", e);
  })
  .finally(async () => {
    try {
      await releaseOrchestratorLease("worker-exit");
    } catch (e) {
      console.error("[sentinelsquad-worker] release failed", e);
    }
