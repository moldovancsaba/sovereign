...
elif [[ -f "$DEFAULT_SENTINELSQUAD_ROOT/package.json" ]]; then
  SENTINELSQUAD_ROOT="$DEFAULT_SENTINELSQUAD_ROOT"
else
  echo "Could not locate SentinelSquad project root."
...
SENTINELSQUAD_LAUNCH_MODE="${SENTINELSQUAD_LAUNCH_MODE:-dev}"

APP_LOG_DIR="$SENTINELSQUAD_ROOT/.sentinelsquad/launcher-logs"
mkdir -p "$APP_LOG_DIR"
APP_LOG="$APP_LOG_DIR/sentinelsquad-app.log"
WORKER_LOG="$APP_LOG_DIR/sentinelsquad-worker.log"
OLLAMA_LOG="$APP_LOG_DIR/ollama.log"
OPENCLAW_LOG="$APP_LOG_DIR/openclaw.log"
BUILD_LOG="$APP_LOG_DIR/sentinelsquad-build.log"
...
echo "=========================================="
echo "       SENTINELSQUAD SYSTEM LAUNCHER"
echo "=========================================="
echo "Project: $SENTINELSQUAD_ROOT"
echo "SentinelSquad URL: $SENTINELSQUAD_APP_URL"
echo "Launch mode: $SENTINELSQUAD_LAUNCH_MODE"
echo "Auto-start OpenClaw gateway: $OPENCLAW_AUTOSTART"
echo "------------------------------------------"

echo "Resetting SentinelSquad listeners (restart mode)..."
kill_listeners_on_port "$SENTINELSQUAD_APP_PORT" "SentinelSquad app"
...
if [[ "$SENTINELSQUAD_LAUNCH_MODE" == "prod" ]]; then
  echo "Building SentinelSquad for production..."
  "$NPM_BIN" run build >"$BUILD_LOG" 2>&1
  echo "Starting SentinelSquad app (production)..."
  "$NPM_BIN" run start >"$APP_LOG" 2>&1 &
else
  echo "Starting SentinelSquad app (development)..."
...
if [[ "$app_ready" -ne 1 ]]; then
  echo "SentinelSquad app failed health check (${SENTINELSQUAD_APP_URL%/}${SENTINELSQUAD_SIGNIN_PATH})."
  echo "See log: $APP_LOG"
  exit 1
fi

echo "SentinelSquad app is listening on port $SENTINELSQUAD_APP_PORT."
...
echo "------------------------------------------"
echo "SentinelSquad launched successfully"
