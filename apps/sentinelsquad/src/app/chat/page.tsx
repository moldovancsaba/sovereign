    ...
    >
      <div className="mb-4 grid gap-3 md:grid-cols-3">
        <div className="rounded-2xl border border-emerald-300/20 bg-emerald-300/10 p-4">
          <div className="text-xs uppercase tracking-wide text-emerald-100/70">Latest successful run</div>
          <div className="mt-2 text-sm text-emerald-50">
            {commandCenterSummary.latestSuccess
              ...
              : "No successful agent run yet."}
          </div>
          {commandCenterSummary.latestSuccess ? (
            <div className="mt-2 text-xs text-emerald-100/75">
              {new Date(commandCenterSummary.latestSuccess.createdAt).toLocaleString()}
              {commandCenterSummary.latestSuccess.meta?.model
                ? ` · ${commandCenterSummary.latestSuccess.meta.model}`
                : ""}
              {commandCenterSummary.latestSuccess.meta?.durationMs != null
                ? ` · ${Math.round(commandCenterSummary.latestSuccess.meta.durationMs)}ms`
                : ""}
            </div>
          ) : null}
        </div>
      ...
      <div className="mb-4 rounded-2xl border border-white/12 bg-white/5 p-4">
        <div className="text-sm font-medium text-white/90">Local operator commands</div>
        <div className="mt-2 grid gap-2 text-xs text-white/65 md:grid-cols-3">
          <div className="rounded-xl border border-white/10 bg-black/10 px-3 py-2">
            <span className="font-mono text-white/85">/agents</span> shows the live squad roster.
          </div>
          <div className="rounded-xl border border-white/10 bg-black/10 px-3 py-2">
