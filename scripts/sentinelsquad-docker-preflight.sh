...
# SentinelSquad Docker portability preflight.
...
check_required_env_key() {
  ...
  if [ -z "$line" ]; then
    fail "Missing required key in apps/sentinelsquad/.env: ${key}. Remediation: copy apps/sentinelsquad/.env.example and set ${key}."
    return
  fi
  value="${line#*=}"
  compact="$(printf "%s" "$value" | tr -d '[:space:]')"
  if [ -z "$compact" ]; then
    fail "Empty required key in apps/sentinelsquad/.env: ${key}. Remediation: set a non-empty value."
    return
  fi
  case "$value" in
...
echo "SentinelSquad Docker portability preflight"
echo "Repo: $REPO_ROOT"
echo
...
if [ -f "$ENV_FILE" ]; then
  pass "SentinelSquad env file exists: apps/sentinelsquad/.env"
  ...
  check_required_env_key "$ENV_FILE" "SENTINELSQUAD_GITHUB_TOKEN"
else
  fail "SentinelSquad env file is missing: apps/sentinelsquad/.env. Remediation: copy apps/sentinelsquad/.env.example to apps/sentinelsquad/.env and fill required keys."
fi

if command -v lsof >/dev/null 2>&1; then
  check_port_listener "$DB_PORT" "SENTINELSQUAD_DB_PORT"
  check_port_listener "$APP_PORT" "SENTINELSQUAD_APP_PORT"
else
