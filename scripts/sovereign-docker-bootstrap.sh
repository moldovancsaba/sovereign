...
# One-command Docker bootstrap for {sovereign}:
...
HEALTH_TIMEOUT_SECONDS="${SOVEREIGN_DOCKER_HEALTH_TIMEOUT_SECONDS:-${SENTINELSQUAD_DOCKER_HEALTH_TIMEOUT_SECONDS:-120}}"
...
echo "{sovereign} Docker bootstrap"
echo "Repo: $REPO_ROOT"
...
dc run --rm --no-deps sovereign-app npx prisma migrate deploy
echo "[PASS] Prisma migrations applied"
echo

echo "[STEP] Starting app service"
dc up -d sovereign-app
wait_for_health "sovereign-app" "$HEALTH_TIMEOUT_SECONDS"
echo
...
if curl -fsS "http://127.0.0.1:${APP_PORT}/signin" >/dev/null; then
  ...
  docker logs --tail 80 sovereign-app || true
  exit 1
fi

echo
echo "[PASS] {sovereign} Docker bootstrap completed."
